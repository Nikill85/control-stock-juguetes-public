...
export class ProveedoresComponent implements OnInit {
  ...
  getProveedores() {
    this.httpClient.get(`https://control-stock-juguetes-1.onrender.com/proveedor`).subscribe((proveedores: Proveedor[]) => {
      console.log("PROVEEDORES: ", proveedores);
      this.proveedores = proveedores;
    });
  }
  crearProveedor() {
    this.httpClient.post(`https://control-stock-juguetes-1.onrender.com/proveedor`, this.nuevoProveedor).subscribe((data: any) => {
      console.log(data);
      if (data.ProveedorID) {
