...
export class ProveedoresComponent implements OnInit {
  ...
  getProveedores() {
    this.proveedoresService.getProveedores().subscribe((proveedores: Proveedor[]) => {
      console.log("PROVEEDORES: ", proveedores);
      this.proveedores = proveedores;
    });
  }
  ...
  crearProveedor() {
    this.proveedoresService.crearProveedor(this.nuevoProveedor).subscribe((data: any) => {
      console.log(data);
      if (data.ProveedorID) {
        this.nuevoProveedor.id_proveedores = data.ClienteID;
        this.proveedores.push(this.nuevoProveedor);
  ...
  actualizarProveedor() {
    this.esEdit = false;
    this.proveedoresService.editProveedor(this.nuevoProveedor).subscribe(respuesta => {
      console.log(respuesta);
      this.getProveedores();
      this.resetForm();
    });
  ...
  // Método para configurar el proveedor para edición
  ...
  eliminarProveedorr(proveedor: Proveedor) {
    this.messageService.confirmMessage(``, `Seguro que desea eliminar el proveedor ${proveedor.nombre}`, `Eliminar`, `warning`)
      .then(r => {
        if (r.isConfirmed) {
          this.eliminarProv(proveedor.id_proveedores); // Asegúrate de pasar el ID del proveedor
        }
  ...
  // Método para eliminar un proveedor usando el servicio
  eliminarProv(id: number) {
    this.proveedoresService.eliminarProveedor(id).subscribe(respuesta => {
      console.log(respuesta);
      this.getProveedores();
    });
  }
