...
router.post('/', async (req: Request, res: Response) => {
    ...
    try {
        await conexion.execute('INSERT INTO proyecto_final.compras (fecha_compra, cantidad,precio_costoUnitario, total_costoCompra) VALUES (?, ?,?,?)', [fecha_compra, cantidad,precio_costoUnitario, total_costoCompra]);
...
router.put('/', async (req: Request, res: Response) => {
    const { id, fecha_compra, cantidad,precio_costoUnitario, total_costoCompra } = req.body; // Acceder al id desde el cuerpo de la solicitud
    try {
